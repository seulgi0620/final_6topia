	...
	//통신이 성공적으로 이루어졌을 때 처리하고 싶은 함수
		...
		$("button[name = 'btn_myChart']").click(function(){
			...
			$.ajax({
				type:"GET",
				url:"../card/cardInsert",
				data : "no="+selectChartNo+"&card_code="+card_code+"&team_code="+team_code, 
				dataType:"HTML",
				success : function(result) {
				//통신이 성공적으로 이루어졌을 때 처리하고 싶은 함수
...
$.ajax({
	type:"GET",
	url:"../card/cardAttach",
	data : "card_code="+card_code+"&team_code="+team_code,
	dataType:"HTML",
	success : function(result) {
		$('#chartbutton').append(result);
	}
      ...
      <div class="modal-footer" style="background-color:#FAE0D4">
        ...
        <!-- 참여자 div -->
         ...
         <%if(partiList!=null){
        	 ...
        	 %>
        	 <button id="<%=parti_code.get(i) %>" name="gkgk" draggable="true" ondragstart="drag(event)" style='background-color:#D9D9D9;height:37px;margin-right:3px' onClick="mouse(id)"> <img id="<%=parti_code.get(i) %>" src="http://192.168.0.8:9000/4People_last2Spring/pds/<%=p_mem_image.get(i) %>"  alt="Cinque Terre" width="30" height="30" ><%=mem_name.get(i) %></button>
        	 <%}} %>
...
<%--       <div id="<%=check_con_code.get(i) %>" class="checkbox" class='min'> --%>
     ...
     <%} }%>
        ...
        <!-- 한마디 div -->
         ...
         %>
         	<img  width="30px" height="30px"  src="http://192.168.0.8:9000/4People_last2Spring/pds/<%=c_mem_image.get(i)%>">
